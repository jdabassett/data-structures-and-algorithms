...
const addPurchases = (arr) => {
  // Solution code here...
  return arr.reduce((acc,item)=>{return acc + item.purchasePrice;},0);
};
...
const returnNames = (arr) => {
  // Solution code here...
  let returnArray = arr.reduce((acc,item,index)=>{
    acc.push(item.name);
    return acc;},[]);
  return returnArray;
};
...
const reversedString = (str) => {
  // Solution code here...
  let array = str.split('');
  let returnArray = array.reduce((acc,item)=>{
    acc.unshift(item);
    return acc;},[]);
...
const countNumberOfChildren = (arr) => {
  let arrayParents = arr.filter((item)=>{return (item.children !== null && item.hasOwnProperty('children'));});
  let arrayArrays = arrayParents.reduce((acc,item)=>{acc.push(item.children); return acc;},[]);
  let sumChildren = arrayArrays.reduce((acc,item)=>{acc+=(item.length); return acc;},0);
  return sumChildren;
};
...
const calculateAverage = (arr) => {
  // Solution code here...
  let sum = arr.reduce((acc,item)=>{acc+=item;return acc;},0);
...
const countPrimeNumbers = (arr) => {
  // Solution code here...
  let count = arr.reduce((acc,item)=>{acc+=(+(isPrime(item))); return acc;},0);
  return count;
};
...
const extractStats = (snorlaxData) => {
  // Solution code here...
  let returnArray = snorlaxData.stats.reduce((acc,item)=>{acc[item.stat.name]=item.baseStat; return acc;},{});
  return returnArray;
};
